...
/** Default number of matches returned when no limit is supplied. */
export const DEFAULT_SEARCH_LIMIT = 5;

/** Hard ceiling on returned matches, regardless of what a caller asks for. */
...
export function searchContext(
  ...
  // A retrieval tool serving an agent should return a bounded, ranked set.
  // Returning every positive match hands the caller an unbounded list whose
  // tail is noise, and grows with the corpus rather than with relevance.
  const effectiveLimit = Math.max(1, Math.min(Math.floor(limit), MAX_SEARCH_LIMIT));
  ...
  // The index is built per call because the record set is an argument rather
  // than server state. For a larger corpus this should be hoisted and reused;
  // at this scale the rebuild is not measurable.
  const index = buildRankingIndex(validated.map((result) => result.record));

  return validated
    .map((result) => ({
      result,
      score: scoreRecord(index, result.record.id, queryTerms),
    }))
