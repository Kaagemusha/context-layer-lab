...
The lab ingests synthetic Markdown documents into organizational records with
explicit ownership, freshness windows, sources, claim-to-source links, and
deterministic provenance receipts. It exposes the records through four
read-only MCP tools and tests the failure modes directly.
...
- Source changes produce deterministic SHA-256 ingestion receipts.
...
ingestion time, so the same source produces the same output. The browser copies
are generated by `npm run demo:sync`; CI fails if either layer drifts.
