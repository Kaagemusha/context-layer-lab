...
flowchart LR
    ...
    R --> W["Public operational replay"]
    D --> W
```

The core is intentionally independent from MCP. `src/context.ts` owns
validation and search; `src/operational-health.ts` owns the replayable
reconciliation rule; `src/tool-handlers.ts` converts context operations into
